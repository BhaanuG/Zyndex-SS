...
import { BookOpen, Search, User, LogOut, ArrowLeft, X, Sparkles, CreditCard } from 'lucide-react';
...
export default function UserLayout({ children }) {
  ...
  const { user, logout, getUrlSafeName, getUrlSafeEmail } = useAuth();
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            {user?.subscriptionPlan && user.subscriptionPlan !== 'FREE' ? (
              <Link
                to={`${baseUrl}/Profile/Billing/Pricing`}
                title="Manage Subscription Plan"
                className="inline-flex items-center gap-1.5 px-3 py-1 bg-gradient-to-r from-orange-50 to-amber-50 text-orange-700 text-xs font-bold rounded-full border border-orange-200/80 uppercase tracking-wider shadow-xs hover:border-orange-300 hover:scale-105 transition-all"
              >
              ...
              <Link
                ...
                className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 text-slate-600 text-xs font-bold rounded-full border border-slate-200 uppercase tracking-wider hover:bg-orange-50 hover:text-orange-600 hover:border-orange-200 transition-all"
              >
                Free Plan · Upgrade
              </Link>
            )}
          ...
          <nav className="flex items-center gap-6">
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Link
                ...
                className={`text-sm font-medium transition-colors flex items-center gap-1.5 ${
                  location.pathname.includes('/Profile') && !location.pathname.includes('/Billing/Pricing')
                    ? 'text-orange-600 font-semibold' 
                    : 'text-gray-600 hover:text-gray-900'
                }`}
              >
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Link
                to={`${baseUrl}/Profile/Billing/Pricing`}
                className={`text-sm font-medium transition-colors flex items-center gap-1.5 ${
                  location.pathname.includes('/Profile/Billing/Pricing')
                    ? 'text-orange-600 font-semibold' 
                    : 'text-gray-600 hover:text-gray-900'
                }`}
              >
