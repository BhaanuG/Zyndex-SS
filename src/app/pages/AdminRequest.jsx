...
export default function AdminRequest() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Submit request to backend first
      await authService.requestAdminAccess(formData);

      // 2. Try to send email (EmailJS failures do not block success)
      let emailSent = true;
      try {
        await emailjs.send(
          EMAILJS_SERVICE_ID,
        ...
        console.error('Admin request email failed to send:', mailError);
        emailSent = false;
      }

      // Show loader with success message
      const msg = emailSent
        ? `Thank you ${formData.displayName}! Your admin account request has been submitted successfully. You'll receive a confirmation at ${formData.email}.`
        : `Thank you ${formData.displayName}! Your admin account request has been registered on our servers, but the confirmation email could not be sent.`;
